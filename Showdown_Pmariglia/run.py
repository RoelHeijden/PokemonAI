...
from Showdown_Pmariglia.config import init_logging
import logging

from Showdown_Pmariglia.teams import load_team
from Showdown_Pmariglia.showdown.run_battle import pokemon_battle
...
from Showdown_Pmariglia.data.mods.apply_mods import apply_mods
...
def parse_configs():
    ...
    config.timer_on = env("TIMER_ON", config.timer_on) == 'True'
...
async def showdown():
    ...
    while True:
        ...
        if config.bot_mode == constants.CHALLENGE_USER:
            await ps_websocket_client.challenge_user(config.user_to_challenge, config.pokemon_mode, team, config.room_name)
