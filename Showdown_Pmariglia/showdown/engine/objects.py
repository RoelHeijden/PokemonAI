...
class Pokemon(object):
    ...
    def calculate_boosted_stats(self):
        return {
            constants.ATTACK: int(boost_multiplier_lookup[self.attack_boost] * self.attack),
            constants.DEFENSE: int(boost_multiplier_lookup[self.defense_boost] * self.defense),
            constants.SPECIAL_ATTACK: int(boost_multiplier_lookup[self.special_attack_boost] * self.special_attack),
            constants.SPECIAL_DEFENSE: int(boost_multiplier_lookup[self.special_defense_boost] * self.special_defense),
            constants.SPEED: int(boost_multiplier_lookup[self.speed_boost] * self.speed),
        }

    def calculate_boosted_stats_for_crits(self):
        if self.attack_boost < 0:
            attack = self.attack
        else:
            attack = int(boost_multiplier_lookup[self.attack_boost] * self.attack)

        if self.defense_boost > 0:
            defense = self.defense
        else:
            defense = int(boost_multiplier_lookup[self.defense_boost] * self.defense)

        if self.special_attack_boost < 0:
            special_attack = self.special_attack
        else:
            special_attack = int(boost_multiplier_lookup[self.special_attack_boost] * self.special_attack)

        if self.special_defense_boost > 0:
            special_defense = self.special_defense
        else:
            special_defense = int(boost_multiplier_lookup[self.special_defense_boost] * self.special_defense)

        return {
            constants.ATTACK: attack,
            constants.DEFENSE: defense,
            constants.SPECIAL_ATTACK: special_attack,
            constants.SPECIAL_DEFENSE: special_defense,
            constants.SPEED: int(boost_multiplier_lookup[self.speed_boost] * self.speed),
        }
