...
from Showdown_Pmariglia import data
from Showdown_Pmariglia.data import all_move_json
from Showdown_Pmariglia.data import pokedex
from Showdown_Pmariglia.data.parse_smogon_stats import MOVES_STRING
from Showdown_Pmariglia.data.parse_smogon_stats import SPREADS_STRING
from Showdown_Pmariglia.data.parse_smogon_stats import ABILITY_STRING
from Showdown_Pmariglia.data.parse_smogon_stats import ITEM_STRING
from Showdown_Pmariglia.data.helpers import get_pokemon_sets
from Showdown_Pmariglia.data.helpers import get_mega_pkmn_name
from Showdown_Pmariglia.data.helpers import PASS_ITEMS
from Showdown_Pmariglia.data.helpers import PASS_ABILITIES
from Showdown_Pmariglia.data.helpers import get_all_likely_moves
from Showdown_Pmariglia.data.helpers import get_most_likely_item
from Showdown_Pmariglia.data.helpers import get_most_likely_ability
from Showdown_Pmariglia.data.helpers import get_most_likely_spread
from Showdown_Pmariglia.data.helpers import get_all_possible_moves_for_random_battle

from Showdown_Pmariglia.showdown.engine.objects import State
from Showdown_Pmariglia.showdown.engine.objects import Side
from Showdown_Pmariglia.showdown.engine.objects import Pokemon as TransposePokemon

from Showdown_Pmariglia.showdown.engine.helpers import remove_duplicate_spreads
from Showdown_Pmariglia.showdown.engine.helpers import get_pokemon_info_from_condition
from Showdown_Pmariglia.showdown.engine.helpers import set_makes_sense
...
from Showdown_Pmariglia.showdown.engine.helpers import common_pkmn_stat_calc
...
class Battle(ABC):
    ...
    def create_state(self):
        ...
        state = State(user, opponent, self.weather, self.field, self.trick_room, self.turn)
        return state
    ...
    def from_team_json(self, team_json):
        ...
        for pokemon_info in team_json:
            ...
            if self.active and self.active.name == name:
                ...
                for mon in self.reserve:
                    if mon.name == name:
                        pokemon = mon
            if pokemon is None:
                logger.error(f"Pokemon {name} does not exist on the player's team")

            for move in pokemon_info['moves']:
                pokemon.add_move(move)

            evs = ''
            for ev in pokemon_info['evs'].values():
                if ev == '':
                    ev = '0'
                evs += ev + ','
            evs = evs[:-1]
            ...
            ivs = ''
            for iv in pokemon_info['ivs'].values():
                if iv == '':
                    iv = 31
                ivs += str(iv) + ','
            ivs = ivs[:-1]
            ...
            pokemon.set_spread(nature, evs, ivs)

            if name == 'shedinja':
                pokemon.hp = 1
                pokemon.max_hp = 1

            pokemon.item = pokemon_info['item']
            pokemon.ability = pokemon_info['ability']
...
class Pokemon:
    ...
    def __init__(self, name: str, level: int, nature="serious", evs=(85,) * 6):
        ...
        speed_min = math.floor((common_pkmn_stat_calc(self.base_stats[constants.SPEED], iv=0, ev=0, level=self.level) + 5) * 0.9)
        speed_max = math.floor((common_pkmn_stat_calc(self.base_stats[constants.SPEED], iv=31, ev=252, level=self.level) + 5) * 1.1)
        self.speed_range = StatRange(min=speed_min, max=speed_max)
    ...
    def set_spread(self, nature, evs, ivs='31,31,31,31,31,31'):
        evs = [int(e) for e in evs.split(',')]
        ivs = [int(e) for e in ivs.split(',')]
        hp_percent = self.hp / self.max_hp
        self.stats = calculate_stats(self.base_stats, self.level, ivs=ivs, evs=evs, nature=nature)
        self.nature = nature
        self.evs = evs
        self.max_hp = self.stats.pop(constants.HITPOINTS)
