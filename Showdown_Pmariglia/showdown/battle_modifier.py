...
from Showdown_Pmariglia.showdown.battle import Pokemon
from Showdown_Pmariglia.showdown.battle import LastUsedMove
from Showdown_Pmariglia.showdown.battle import DamageDealt
from Showdown_Pmariglia.showdown.battle import StatRange
...
from Showdown_Pmariglia.showdown.engine.find_state_instructions import get_effective_speed
from Showdown_Pmariglia.showdown.engine.damage_calculator import calculate_damage
from Showdown_Pmariglia.showdown.engine.objects import boost_multiplier_lookup
...
def check_speed_ranges(battle, msg_lines):
    ...
    switches = [s for s in msg_lines if s.startswith('|switch|')]
    ...
    if len(switches) == 2:
        ...
        priority_could_be_modified = False

    # check speed if both pokemon attack with equal priority OR a pokemon uses a switching move
    elif (len(moves) == 2 and priorities_equal) or (switching_move_used and len(switches) == 0):
        bot_went_first = moves[0][0].startswith(battle.user.name)

        # check if priority has been modified by e.g. Prankster + status move
        if bot_went_first:
            priority_could_be_modified = can_have_priority_modified(battle, battle.user.active, moves[0][1][constants.ID])
        else:
            priority_could_be_modified = can_have_priority_modified(battle, battle.opponent.active, moves[0][1][constants.ID])

    # one pokemon knocked the other out with a neutral priority move before it could move
    # NOTE: this check is inaccurate if the pokemon that did not get to move used a negative priority move
    elif len(moves) == 1 and len(switches) == 0 and moves[0][1][constants.PRIORITY] == 0:
        bot_went_first = moves[0][0].startswith(battle.user.name)
        if bot_went_first:
            priority_could_be_modified = can_have_priority_modified(battle, battle.user.active, moves[0][1][constants.ID])
        else:
    ...
    # otherwise do not check the speed_range
    else:
        return
    ...
    if bot_went_first:
        if battle.opponent.active.speed_range.max > speed_threshold:
        ...
        if battle.opponent.active.speed_range.min < speed_threshold:
            battle.opponent.active.speed_range = StatRange(
                min=speed_threshold,
