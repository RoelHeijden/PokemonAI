...
from Showdown_Pmariglia.data.helpers import get_standard_battle_sets
from Showdown_Pmariglia import constants
from Showdown_Pmariglia import config
from Showdown_Pmariglia.showdown.engine.evaluate import Scoring
from Showdown_Pmariglia.showdown.battle import Pokemon
from Showdown_Pmariglia.showdown.battle import LastUsedMove
from Showdown_Pmariglia.showdown.battle_modifier import async_update_battle
from Showdown_Pmariglia.teams import load_team, team_converter
from Showdown_Pmariglia.showdown.websocket_client import PSWebsocketClient
...
async def initialize_battle_with_tag(ps_websocket_client: PSWebsocketClient, set_request_json=True):
    ...
    # battle_module = importlib.import_module('showdown.battle_bots.safest.main')
    # battle_module = importlib.import_module('showdown.battle_bots.nash_equilibrium.main')
    battle_module = importlib.import_module('KetchAI.main')
...
async def start_standard_battle(ps_websocket_client: PSWebsocketClient, pokemon_battle_type):
    ...
    if battle.generation in constants.NO_TEAM_PREVIEW_GENS:
        ...
        if config.opp_team_name:
            opp_team = load_team(config.opp_team_name)
            opp_team = team_converter.packed_to_json(opp_team)
            battle.opponent.from_team_json(opp_team)
